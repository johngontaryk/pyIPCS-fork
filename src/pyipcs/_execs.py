"""
pyIPCS Session Execs
"""

from ._version import __version__

# IPCS Version - REXX to write PYIPCS=<pyIPCS_version>
IPCSVERS = f"""/* REXX */
SAY 'PYIPCS={__version__}'
EXIT 0
"""

# IPCS Driver - CLIST to run IPCS subcommands
IPCSRUN = """PROC 0 SUBCMD() SETDEFPARMS()

IPCS NOPARM                 /* ENTER THE IPCS SUBCOMMAND */
                            /* TO START AN IPCS SESSION  */
IF &LASTCC>8 THEN EXIT CODE(&MAXCC)

IF &LENGTH(&STR(&SETDEFPARMS)) > 0 THEN DO   /* RUN SETDEF IF PARAM GIVEN */
  SETDEF NOLIST LOCAL &SETDEFPARMS
END
